...
@section('title', 'Tambah Tenaga Kependidikan - SMAN 24 Bandung')
@section('breadcrumb', 'Tambah Tendik')

@section('content')
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <h2 class="text-xl font-bold text-slate-900">Tambah Data Tenaga Kependidikan (Tendik)</h2>
    ...
    <form action="{{ route('admin.staff.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        @csrf

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Nama Lengkap</label>
                <input type="text" id="name" name="name" value="{{ old('name') }}" required placeholder="Masukkan nama lengkap staf..." class="form-input-custom">
                @error('name')
            ...
            <div>
                ...
                <input type="text" id="position" name="position" value="{{ old('position') }}" required placeholder="contoh: Kepala Tata Usaha, Staff Laboratorium..." class="form-input-custom">
                @error('position')
                    <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="tu@sman24bdg.sch.id" class="form-input-custom">
            </div>
            <div>
