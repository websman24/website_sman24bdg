...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="6" required placeholder="Tulis isi pengumuman lengkap..." class="form-input-custom">{{ old('content', $announcement->content) }}</textarea>
            @error('content')
                <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
