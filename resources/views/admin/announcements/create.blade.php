...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form action="{{ route('admin.announcements.store') }}" method="POST" class="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="flex items-center pt-6">
                <label class="flex items-center gap-2 cursor-pointer select-none text-xs font-bold text-slate-700">
                    <input type="checkbox" name="is_pinned" value="1" class="w-4 h-4 rounded border-slate-300 text-amber-500 focus:ring-amber-500">
        ...
        <div>
            ...
            <textarea id="content" name="content" rows="6" required placeholder="Tulis isi pengumuman lengkap..." class="w-full px-4 py-3 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-emerald-700 focus:outline-none">{{ old('content') }}</textarea>
            @error('content')
                <p class="text-xs text-rose-600 mt-1">{{ $message }}</p>
            @enderror
        </div>
