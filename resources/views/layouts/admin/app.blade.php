    ...
    <div class="h-screen flex flex-col md:flex-row overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
               ...
               x-data="{
                   ...
                   openAkademik: {{ request()->routeIs(['admin.profiles.*', 'admin.teachers.*', 'admin.staff.*', 'admin.achievements.*', 'admin.extracurriculars.*', 'admin.osis.*']) ? 'true' : 'false' }},
            ...
            <nav class="flex-1 px-3 mt-3 space-y-2 text-xs font-semibold overflow-y-auto pb-6 custom-scrollbar">
                ...
                <div class="space-y-1">
                    <button @click="openAkademik = !openAkademik" 
                            type="button"
                            class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl transition-all font-bold {{ request()->routeIs(['admin.profiles.*', 'admin.teachers.*', 'admin.staff.*', 'admin.achievements.*', 'admin.extracurriculars.*', 'admin.osis.*']) ? 'bg-emerald-900/60 text-amber-400 border border-emerald-700/60 shadow-sm' : 'text-slate-300 hover:bg-slate-800/80 hover:text-white' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-7 h-7 rounded-lg bg-emerald-800/80 text-amber-400 flex items-center justify-center shadow-inner shrink-0">
                    ...
                    <div x-show="openAkademik" 
                        ...
                        <a href="{{ route('admin.osis.index') }}" class="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] transition-all {{ request()->routeIs('admin.osis.*') ? 'bg-emerald-800 text-white font-extrabold shadow-sm' : 'text-slate-400 hover:bg-slate-800/60 hover:text-slate-200' }}">
